...
function happensColumn(data, col, quorum) {
  const column = [];
...
function participationTable(data, times, meetings, quorum) {
  const probs = new Array(data.length);
  for (let k = 0; k < meetings; k++) {
    const t = times[k % times.length];
    const h = happensColumn(data, t, quorum);
    for (let i = 0; i < data.length; i++) {
      const p = h * data[i][t];
      if (!probs[i]) probs[i] = [];
      addEvent(probs[i], p);
...
function interactionTable(data, times, meetings, quorum) {
  const probs = [];
  for (let k = 0; k < meetings; k++) {
    const t = times[k % times.length];
    const h = happensColumn(data, t, quorum);
...
function analyze(raw, times, meetings, values, quorum) {
  // Convert raw data table to table of actual probabilities
  const data = raw.map(row => row.map(x => values[x]));

  const info = {
    count: times.length,
    times,
    probs: {
      participation: participationTable(data, times, meetings, quorum),
      interaction: interactionTable(data, times, meetings, quorum)
    }
  };
  info.scores = {
