...
function analyze(raw, times, meetings, values, quorum) {
  ...
  const data = raw.values.map(row => row.map(x => values[x]));

  // Compute an array of meeting reaching quorum probabilites
  const happened = [];
  for (let t = 0; t < data[0].length; t++) {
    // Select a column of probabilites and compute the chance of reaching quorum
    const column = data.map(row => row[t]);
    happened[t] = happens(column, quorum);
  }
  ...
  const info = {
    ...
    times,
    probs: {
      participation: participationTable(data, times, meetings, happened),
      interaction: interactionTable(data, times, meetings, happened)
    }
  };
  info.scores = {
