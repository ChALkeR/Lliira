...
function utility(table, base) {
  const maximum = table[0].slice().fill(table.length);
  const actual = table[0].slice().fill(0);
  for (let i = 0; i < table.length; i++) {
    for (let j = 0; j < table[i].length; j++) {
      actual[j] += table[i][j];
    }
  }
  const utils = flatten(actual, base);
  const limit = flatten(maximum, base);
  return utils / limit;
}
...
function tableParams(table) {
  // Compute average percentage per entry
  const scores = table.map(row => total(row) / row.length);
  const vector = scores.slice().sort().reverse();
  const res = {
    scores,
    vector,
    avg: total(scores) / scores.length,
    min: Math.min(...scores),
    stdev: pstdev(scores), // ranging from 0 to 0.5, lower is better
    fair: fairness(table),
    //u2: utility(table, 0.5),
    n5: scores.filter(x => x >= 0.5).length
  };
  if (scores.length < 36) {
    res.u05 = Array(20).fill(0)
      .map((z, i) => scores.filter(x => x >= 0.05 * i).length.toString(36))
      .join('');
  }
  return res;
}
...
function analyze(data, times, meetings, values, quorum) {
  ...
  const info = {
    ...
    meetings: {
      probs: times.map(t => data.happens[t]),
      avg: times.map(t => total(data.probabilites.map(r => r[t])))
    }
  };
  info.scores = {
