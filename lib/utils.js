...
function percentages(obj) {
  return obj.scores.map(x => x * 100).sort((a, b) => a - b).reverse();
}
...
function formatPretty(info) {
  const lines = formatShort(info)
    .replace(/, (participation|interaction)/g, '\n  $1')
    .split('\n');
  const perc = percentages(info.scores.participation).map(x => Math.round(x));
  const percLine = format(perc).slice(2, -2).replace(/ /g, '');
  lines.splice(2, 0, `  percentages: ${percLine}`);
  lines.push(`  meetings: ${format({
    probs: info.meetings.probs.map(x => roundTo(x, 1e3)),
    avg: info.meetings.avg.map(x => roundTo(x, 1e2))
  })}`);
  return lines.join('\n');
}
