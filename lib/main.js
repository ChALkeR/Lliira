...
function bruteforceOne(data, size) {
  ...
  for (let base = step ** steps; base >= 1 - 1e-3; base /= step) {
    const baseV = Array(len).fill().map((x, i) => base ** i);
    for (const type of ['participation', 'interaction']) {
      const best = { score: 0 };
      for (const info of infos) {
        const vector = info.scores[type].scores.sort().reverse();
        const multiplied = vector.map((x, i) => x * baseV[i]);
        const score = multiplied.reduce((a, b) => a + b, 0);
        if (score > best.score) {
          best.score = score;
